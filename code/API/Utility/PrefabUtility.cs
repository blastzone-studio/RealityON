...
{
	public static GameObject CreateGameObject( string path, GameObject parent = null)
	{
		PrefabFile prefabFile = ResourceLibrary.Get<PrefabFile>( path );

		if ( prefabFile == null )
		{
			Log.Warning( $"LoadFromFile: Couldn't find {path}" );
			return null;
		}

		return CreateGameObject( prefabFile, parent);
	}
	...
	public static GameObject CreateGameObject( PrefabFile prefabFile, GameObject parent = null )
	{
		...
		if ( Consts.Debug )
			Log.Warning( $"~debug~[PrefabUtility] New gameobject from a prefab has been created {go}" );
